...
func additionalSeedCatalogBulk(now time.Time) []domain.Content {
	specs := []seedSpec{
		...
		{"cnt_research_022", "Research Reading: Comparison Detail", "reading", "intermediate", "The control condition showed the full sentence at once, while the support condition highlighted one core chunk before the modifiers appeared.", "Comparison between two conditions"},
		{"cnt_research_023", "Research Listening: Speaker Summary", "listening", "intermediate", "In short, the interface did not teach more vocabulary, but it did help learners keep the sentence long enough to understand it.", "Short oral summary of the main result"},
		{"cnt_research_024", "Research Listening: Measurement Reminder", "listening", "intermediate", "After each passage, participants rated overload, repeated the main point, and noted where they lost track of the sentence.", "Measurement reminder with three actions"},
		{"cnt_research_025", "Research Speaking: Motivation in Steps", "speaking_template", "intermediate", "The motivation is practical. Many learners know the words. They still lose the sentence when details pile up.", "Motivation template for research speaking"},
		{"cnt_research_026", "Research Speaking: Result in Steps", "speaking_template", "intermediate", "The result has two parts. Overload went down. Summary quality went up.", "Result template using two short claims"},
		{"cnt_research_027", "Research Reading: Design Principle", "reading", "intermediate", "The design principle was simple: do not force the learner to carry every detail while the main meaning is still unstable.", "Design principle in one sentence"},
		{"cnt_research_028", "Research Reading: Broader Impact", "reading", "intermediate", "This approach may also help meetings and lectures because the same overload problem appears outside language classes.", "Broader impact statement"},
		{"cnt_meeting_017", "Meeting Reading: Ask for Today’s Goal", "reading", "intermediate", "Before we continue, could someone restate today's goal in one short sentence?", "Meeting question for goal reset"},
		{"cnt_meeting_018", "Meeting Reading: Confirm Responsibility", "reading", "intermediate", "Just to confirm, are you responsible for the slides, and am I responsible for the final notes?", "Responsibility confirmation question"},
		{"cnt_meeting_019", "Meeting Listening: Review the Draft", "listening", "intermediate", "Please read the draft once tonight and mark only the parts that block understanding.", "Instruction focused on blocking points"},
		{"cnt_meeting_020", "Meeting Listening: Start with the Risk", "listening", "intermediate", "Let us start with the main risk first, and then we can discuss optional improvements if there is time.", "Meeting cue that prioritizes risk first"},
		{"cnt_meeting_021", "Meeting Speaking: State One Concern", "speaking_template", "intermediate", "I have one concern. The wording is still heavy. Users may lose the main point too early.", "Short template for stating one concern"},
		{"cnt_meeting_022", "Meeting Speaking: Ask for a Clear Decision", "speaking_template", "intermediate", "I want to be clear. Is today's decision to ship now or wait one more week?", "Decision clarification in short units"},
